auth.onAuthStateChanged(user => {
    if (user) {
        db.collection('proyectos').orderBy("uploadTime", "desc").get().then(snapshot => {
...
const showFiles = (data) => {
    ...
    data.forEach(doc => {
        ...
        var pathReference = storage.ref('files/proyectos/' + filename);

        console.log(pathReference); */
        var storageRef = firebase.storage().ref('');
        url = storageRef.child('files/proyectos/' + filename).getDownloadURL().then((url) => {
...
function uploadFile() {
    ...
    fileButton.addEventListener('change', (e) => {
        ...
        var storageRef = firebase.storage().ref('files/proyectos/' + file.name);
