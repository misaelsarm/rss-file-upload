...
function render() {
    db.collection('recursos-humanos').get().then((snapshot) => {
        let fileList = document.getElementById("fileList");
        snapshot.docs.forEach(doc => {
            const filename = doc.data().fileName;
            var storageRef = firebase.storage().ref('');
            url = storageRef.child('files/recursos-humanos/' + filename).getDownloadURL().then((url) => {
                // console.log(url)
                ...
                eliminar.addEventListener('click', (e) => {
                    ...
                    db.collection('recursos-humanos').doc(id).delete()
                    var deleteRef = storageRef.child('files/recursos-humanos/' + filename);
...
/* function showDepartments() {
    recursos-humanos = document.getElementById("recursos-humanos")
} */
...
function uploadFile() {
    ...
    fileButton.addEventListener('change', (e) => {
        ...
        var storageRef = firebase.storage().ref('files/recursos-humanos/' + file.name);
        ...
        //console.log(date);
        db.collection('recursos-humanos').add({
            fileName: file.name,
