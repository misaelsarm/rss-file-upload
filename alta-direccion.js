...
function render() {
    db.collection('alta-direccion').get().then((snapshot) => {
        let fileList = document.getElementById("fileList");
        snapshot.docs.forEach(doc => {
            const filename = doc.data().fileName;
            var storageRef = firebase.storage().ref('');
            url = storageRef.child('files/alta-direccion/' + filename).getDownloadURL().then((url) => {
                // console.log(url)
                ...
                eliminar.addEventListener('click', (e) => {
                    ...
                    db.collection('alta-direccion').doc(id).delete()
                    var deleteRef = storageRef.child('files/alta-direccion/' + filename);
                    deleteRef.delete().then(() => {
                        window.location.href = "alta-direccion.html"
                    })
                })
            })
        })
...
/* function showDepartments() {
    alta-direccion = document.getElementById("alta-direccion")
} */
...
function uploadFile() {
    ...
    fileButton.addEventListener('change', (e) => {
        ...
        var storageRef = firebase.storage().ref('files/alta-direccion/' + file.name);
        ...
        //console.log(date);
        db.collection('alta-direccion').add({
            fileName: file.name,
