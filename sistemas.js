...
function render() {
    db.collection('sistemas').get().then((snapshot) => {
        let fileList = document.getElementById("fileList");
        snapshot.docs.forEach(doc => {
            const filename = doc.data().fileName;
            var storageRef = firebase.storage().ref('');
            url = storageRef.child('files/sistemas/' + filename).getDownloadURL().then((url) => {
                // console.log(url)
                ...
                eliminar.addEventListener('click', (e) => {
                    ...
                    db.collection('sistemas').doc(id).delete()
                    var deleteRef = storageRef.child('files/sistemas/' + filename);
...
function uploadFile() {
    ...
    fileButton.addEventListener('change', (e) => {
        ...
        var storageRef = firebase.storage().ref('files/sistemas/' + file.name);
