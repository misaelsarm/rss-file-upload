...
function render() {
    db.collection('auditoria-interna').get().then((snapshot) => {
        let fileList = document.getElementById("fileList");
        snapshot.docs.forEach(doc => {
            const filename = doc.data().fileName;
            var storageRef = firebase.storage().ref('');
            url = storageRef.child('files/auditoria-interna/' + filename).getDownloadURL().then((url) => {
                // console.log(url)
                ...
                eliminar.addEventListener('click', (e) => {
                    ...
                    db.collection('auditoria-interna').doc(id).delete()
                    var deleteRef = storageRef.child('files/auditoria-interna/' + filename);
                    deleteRef.delete().then(() => {
                        window.location.href = "auditoria-interna.html"
                    })
                })
            })
        })
...
/* function showDepartments() {
    auditoria - interna = document.getElementById("auditoria-interna")
} */
...
function uploadFile() {
    ...
    fileButton.addEventListener('change', (e) => {
        ...
        var storageRef = firebase.storage().ref('files/auditoria-interna/' + file.name);
        ...
        //console.log(date);
        db.collection('auditoria-interna').add({
            fileName: file.name,
